...
唯一"才咬得动。真仓库 847 条锁目实测（`check_doc_lines.py` 文件头也是这几个数）：
两个尾部锚点里**至少一个**唯一的 766 条（90.4%）、一个都不唯一的 81 条（9.6%）。
那 9.6% 会走 S5 那一格：`[刷]` + 一行 `[记] 判不了 N 条`。**"判不了"必须自己
...
def main():
    ...
    print("-" * 88)
    bad = 0
    for name, src, doc, mut, want, want_red in SCENES:
        ...
        try:
            ...
            rc, out = run_tool(args.tool, tmp, ["--update"])
