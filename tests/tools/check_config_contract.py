...
三条判据，每条都能当场变成红的：
  ...
  1. **ABI 形态的正例**：拿包里的头 + 包里的 dll，用**裸编译器调用**编一个消费方
     ...
     布局对不上时内联访问器**编得过、链得过**，然后落在两种落点之一：访问器读错
     偏移、返回一个天文数字（`1073741824`，本仓最初那条症状），或者**直接崩** ——
     MSVC 上实测落的是 `0xC0000409`，stdout 一个字都没有。
     **判据不能只认前一种**：写"读数必须是个怪值"的话，崩掉那一档会让用例永远等
     不到读数，看起来像用例本身坏了。这里断言的是"跑出来的数 == 0"，两种落点都红。
...
用法（**没有** `--config` —— 构建配置由 `--tree` 决定，多配置生成器下要传 Release
树；照抄一条不存在的选项会让 argparse 以 rc=2 停下来，**一条判据都不跑**，而输出
看着像"配置不对"）：
