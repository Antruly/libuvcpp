...
（本仓吃过一次这个亏：`n == 1` 等价的重构单独落地时，变异**恒绿**）。
  ...
  M4  `connection_count_at(i)` 不看 i，一律答 0 号
                                → 红（实测：`每条连接都记在它自己那一格上
                                   ...
                                   （0+0 vs 1）` 三条一起响）
                                   抓手换过两次，都记在这里：
                                   ① 最初那条写在"16 条请求全跑完"之后，那时
                                      总数是 0，`0 == 0+0` 对坏实现一样成立 ——
                                      **恒真**，第一次跑没抓住；
                                   ② 补的"处理函数里采样 + `at1 >= 1` 自断言"
                                      有牙；但**同时留下的**那条「逐格之和 ==
                                      `connection_count()`」既恒真（同上是逐格
                                      求和）又会**时红时绿**（四条读不是一个快
                                      照：本机 7567.6 万采样里双向撕裂 138/152
                                      次，`worst(total=1 sum=0)`）⇒ 2026-09-24
                                      在 CI 的 ssl(windows) 上真红了，已删。
                                      删它**没有拔牙**：删完注入 M4 重跑，上面
                                      那三条照样把它抓住（`cell_ok` 那条采在
                                      `on_connection` 里，连接一定活着，是确定
                                      性承重的那条）。
                                   ③ 试过在 `n == 1` 对照组里加一条"处理函数内
                                      `at1 == 0`"当抓手 —— **是死的**：
                                      `connection_count_at` 对越界下标先
                                      `return 0`（`loops_.size() == 1` 时下标 1
                                      根本走不到被变异的那个 `return`）⇒ 注入
                                      M4 它不响。已撤掉，不留摆设。
