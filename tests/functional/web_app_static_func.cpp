...
/// 64 KiB 上限（`internal_alloc_cb` 用的是 libuv 建议的尺寸），请求就会**跨在读边界
/// 上**。那是**另一个**判据（`web_app_read_boundary_func.cpp`，那里专门灌 1100 条、
/// 还换两种填充长度把边界挪到别的请求上），不该混进这一条 —— 每批只灌两三 KB，
/// 服务端从空缓冲开始读，永远碰不到那个边界，于是这个文件红了就一定是静态路由
/// 自己的事。
///
/// **不是"跨边界的请求会被丢掉"的绕行**：那个缺陷已经修掉了（`uvcpp_http_server.cpp`
/// 的 keep-alive 重置改为按**消息尾**而不是按**读**判断，单消息暂存改在 message-begin
/// 清）。这里分批纯粹是"一个文件只打一件事"。
///
