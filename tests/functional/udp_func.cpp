...
int main() {
  ...
  client_stop_async.init([&client_loop](uvcpp_async *a) { client_loop.stop(); },
                         &client_loop);
      ...
      [&client, &port_future, &server_stop_async, &client_loop,
       &success](uvcpp_async *a) {
        ...
        client.recv_start(
            ...
            [&server_stop_async, &client_loop, &success, b,
             timer](uvcpp_udp *u, ssize_t nread, const uv_buf_t *buf,
        ...
        timer->start(
            [dest, &client](uvcpp_timer *t) {
              uv_buf_t *b = (uv_buf_t *)t->get_data();
              ...
              client.send(sreq, b, 1, (const sockaddr *)&dest,
                          [&](uvcpp_udp_send *r, int status) {
                            if (status < 0) {
                              std::cout
                                  << "[functional udp] client send error : "
  ...
  std::thread server_thread([&]() {
    ...
    server_stop_async.init(
        [&server_loop](uvcpp_async *a) { server_loop.stop(); }, &server_loop);
    ...
    server.recv_start(
        ...
        [&](uvcpp_udp *u, ssize_t nread, const uv_buf_t *buf,
            const sockaddr *addr, unsigned flags) {
          if (nread > 0) {
            std::string s(buf->base, (size_t)nread);
            ...
            u->send(req, echo, 1, addr, [&](uvcpp_udp_send *r, int status) {
              if (status < 0) {
                std::cout << "[functional udp] server echo send error : "
                          << uv_strerror(status) << std::endl;
              }
              std::cout << "[functional udp] server echo send status=" << status
                        << std::endl;
              uv_buf_t *echo = (uv_buf_t *)r->get_data();
              uvcpp_buf::free_buf(echo);
              uvcpp_free(echo);
              delete r;
            });
          }
    ...
    std::cout << "[functional udp] server listening (recv started) port="
              << port_after << std::endl;

    // wait until client has created its ready-async, then notify client to
    // start
  ...
  std::thread client_thread(
      [&client_loop]() { client_loop.run(UV_RUN_DEFAULT); });
  ...
  std::cout << "[functional udp] done received="
            << (success.load() ? "true" : "false") << std::endl;
