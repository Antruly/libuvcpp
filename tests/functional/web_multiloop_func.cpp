 ...
 * 不存在）；POSIX 那条 `dup()` 路 —— 它在 Windows 上**根本编不到**（在
 * `uvcpp_socket_handoff.cpp:65` 的 `#else` 里），本机跑的是**同一函数的 Windows
 * 支**（`WSADuplicateSocketW`）。CI 上也只有 **macOS** 腿真跑到它：Linux 侧
 * `UV_TCP_REUSEPORT` 绑得上 ⇒ `fanout_` 为真 ⇒ 永不走转手。不要写成"两端都验过"。
 */
