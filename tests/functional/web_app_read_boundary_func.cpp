 ...
 *
 *  1. **读在消息中间结束** ⇒ 下一次读开头对一条解析到一半的报文调 `llhttp_init`，
 *     半个状态被抹掉。断在报文末尾那个空行上 ⇒ 那条请求**静默消失**（不报错、不回、
 *     也不进任何表）；断在头部中间 ⇒ `HPE_INVALID_METHOD` ⇒ 400 关连接 ⇒ 它之后
 *     整段不回。这个现象最早是在 `web_app_static_func.cpp` 的 `raw_batch` 里撞见的
