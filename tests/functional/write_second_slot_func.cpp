 ...
 * **这条不是假设出来的**：本判据的第一版读的是 `get_stats()` 的**在用块数**，
 * [0] 当场红给我看 —— 那时它只在 span / 大对象两条冷路径上 +1，从两条缓存**命中**
 * 的分配一个都不记，而 `free_mem()` 每次释放都 -1，于是稳态下只减不增、漂成负数
 * （实测 `-75`），对 64 字节的分配完全不响应。那个缺陷已经修掉（`alloc()` 三条
 * 出口收敛成一处记账，`expand_memory_pool_func.cpp` 的 `stats_in_use_follows_alloc`
 * 看着它）；本用例继续读总字节数，是因为它是**空闲量**的直接读数，不依赖记账次数。
 *
