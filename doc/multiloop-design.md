...
worker[k] 线程：loop[k].run(UV_RUN_DEFAULT)
  └─ 邮箱回调里排空（src/net/uvcpp_loop_worker.h:146-146 起）
...
| 容器 | 位置 | 触碰频率 |
|---|---|---|
| `uvcpp_http_server::contexts_` | `src/web/uvcpp_http_server.h:1103` | **每请求**（`.cpp` 里 52 处引用）—— **已切开**（§4.1） |
| ~~`uvcpp_web_app::inflight_`~~ → `loop_slot::inflight` | `src/webapp/uvcpp_web_app.h:1961`（**已切开**，W4 / 2c） | **每请求** |
| ~~`uvcpp_web_app::upgraded_`~~ → `loop_slot::upgraded` | `src/webapp/uvcpp_web_app.h:2020`（**已切开**，W4 / 2c） | 每次 WS 升级 |
...
> （`src/webapp/uvcpp_web_app.h:1866-2091`）里，`loops_`（`src/webapp/uvcpp_web_app.h:2124`）
> 是"每循环一格"的向量，`slot_here()`（`src/webapp/uvcpp_web_app.cpp:2375-2391`）
> 按**调用线程**取格 —— 认不出是哪条循环的线程时给 0 号（§5.2）。
> `n == 1` 走快路直接回 `loops_[0]`，**一次锁都不多加**，与今天逐字相同。
> `post()` 现在是 `src/webapp/uvcpp_web_app.cpp:2324-2336`，`loop()` 是
> `src/webapp/uvcpp_web_app.cpp:2367-2373`，`on_loop_thread()` 是
> `src/webapp/uvcpp_web_app.cpp:2318-2322`。

### 4.1 好消息：大部分切分是"多建几个对象"，不是"给容器加锁"

`uvcpp_http_server` 在自己的构造函数里 `new uvcpp_tcp_server`（`src/web/uvcpp_http_server.cpp:68-70`），
...
> （`src/web/uvcpp_http_server.h:1103`），索引 = `uvcpp_tcp_client::loop_index()`；
...
共享冻结的那份装：`router_`（`src/webapp/uvcpp_web_app.h:1403`）、`middlewares_`（`src/webapp/uvcpp_web_app.h:1404`）、
`ws_router_`/`ws_handlers_`（`src/webapp/uvcpp_web_app.h:1466-1467`）、`stream_router_`（`src/webapp/uvcpp_web_app.h:1482`）、`upload_routes_`
（`src/webapp/uvcpp_web_app.h:1498`）。冻结期之后多线程只读，**不需要锁**。

> **理由更正（2026-09-22，外部复核）：这条的理由**不是**"写点都在 `start()` 里"。**
> `chain_storage_.push_back`（`src/webapp/uvcpp_web_app.cpp:3308`）与
> `chain_cache_[...]`（`src/webapp/uvcpp_web_app.cpp:3310`）确实在 `start()` 里，但**那两个函数在请求路径上可达**：
> `sync_chains()`（`src/webapp/uvcpp_web_app.cpp:3260`）被 `src/webapp/uvcpp_web_app.cpp:1269`（流式那条路）与 `src/webapp/uvcpp_web_app.cpp:3193`（派发前）调用，
> `build_chain()`（`src/webapp/uvcpp_web_app.cpp:3286`）被 `src/webapp/uvcpp_web_app.cpp:1271` 与 `src/webapp/uvcpp_web_app.cpp:3215` 调用。让这些写**离开**请求路径的是
...
> `init_on_loop_thread()`（`src/webapp/uvcpp_web_app.cpp:2176`）是**循环线程的入口**，
> 而访问日志那句 `middlewares_.insert(middlewares_.begin(), web_middleware_access_log())`
> （`src/webapp/uvcpp_web_app.cpp:1901`）与 `++middleware_gen_`
> （`src/webapp/uvcpp_web_app.cpp:1902`）**当天就在这个入口里**（今天已经搬走，见下面的已落地）。
> n>1 时这个函数**每条循环各跑一次** ⇒
> ① 插进 **n 份**访问日志中间件（每请求记 n 次。本库对这一档有读数：`off → info`
> 是 +17~19% rps、每请求 19 → 25 次分配）；
> ② 更要紧的是**竞态** —— `middlewares_` / `middleware_gen_` 在请求路径上被**读**
> （`src/webapp/uvcpp_web_app.cpp:3275`、`src/webapp/uvcpp_web_app.cpp:3299-3301`、`src/webapp/uvcpp_web_app.cpp:3320`/`src/webapp/uvcpp_web_app.cpp:3335`/`src/webapp/uvcpp_web_app.cpp:3354`），而 n 条循环**不同时起跑**：
...
`init_process_once()`（`src/webapp/uvcpp_web_app.cpp:1792`），每循环那半仍是
`init_on_loop_thread()`（`src/webapp/uvcpp_web_app.cpp:2176`）。两处调用点都按
**先每进程、后每循环**的次序调它们（`src/webapp/uvcpp_web_app.cpp:1610-1676` 与
`src/webapp/uvcpp_web_app.cpp:1751-1790`，理由见 §4.1.1 甲）。上面 ①② 两条后果随之消失：
...
| 成员 | 位置 |
|---|---|
| `router_` | `src/webapp/uvcpp_web_app.h:1403` |
| `middlewares_` + `middleware_gen_` | `src/webapp/uvcpp_web_app.h:1404` / `src/webapp/uvcpp_web_app.h:1783` |
| `ws_router_` / `ws_handlers_` | `src/webapp/uvcpp_web_app.h:1466` / `src/webapp/uvcpp_web_app.h:1467` |
| `stream_router_` / `stream_routes_seen_` | `src/webapp/uvcpp_web_app.h:1482` / `src/webapp/uvcpp_web_app.h:1485` |
| `upload_routes_` | `src/webapp/uvcpp_web_app.h:1498` |
| `upload_dir_` / `upload_dir_real_` / `upload_dir_unsafe_` | `src/webapp/uvcpp_web_app.h:1504` / `src/webapp/uvcpp_web_app.h:1513` / `src/webapp/uvcpp_web_app.h:1524` |
| `static_roots_real_` | `src/webapp/uvcpp_web_app.h:1521` |
| 上传上限族（6 个） | `src/webapp/uvcpp_web_app.h:1544-1549` |
| 连接钩子族（4 个） | `src/webapp/uvcpp_web_app.h:1794-1800` |
| `work_limit_` | `src/webapp/uvcpp_web_app.h:1413` |

**其中 `middlewares_` 原本是这一格里唯一在错误的位置被写的**：写点
（`src/webapp/uvcpp_web_app.cpp:1901` 与 `src/webapp/uvcpp_web_app.cpp:1902`）
当时落在循环线程的入口里，而所有者是**进程**（上面那条更正二）。
**2026-09-22 已落地**：这两句所属的函数现在是 `init_process_once()`
（`src/webapp/uvcpp_web_app.cpp:1792`）—— **行号没动，换的是函数**。甲这一格其余各笔
...
| 成员 | 位置 | 为什么是进程级 |
|---|---|---|
| `bound_port_` | `src/webapp/uvcpp_web_app.h:2126` | 一个 app 一个监听端口（`bind()` 只调一次）。 |
| `started_once_` | `src/webapp/uvcpp_web_app.h:1815` | "这个 app 起过了"，与几条循环无关。 |
| `stopping_` | `src/webapp/uvcpp_web_app.h:1831` | 它答的是"这次停机请求受理过了没有"（幂等用 `exchange`，`src/webapp/uvcpp_web_app.cpp:2201`）。`shutdown_phase_` 才是"我这条循环走到第几步" —— **两者今天挨着写，很容易被一起搬**。 |
| `running_` / `loop_started_` | `src/webapp/uvcpp_web_app.h:1830` / `src/webapp/uvcpp_web_app.h:1829` | 它们是**聚合量**："有没有任何一条循环在跑"。`loop_started_` 的语义在 W4 里正式改成"**至少还有一条循环在跑**"（由 `std::atomic<int> loops_running_` 记数）。⚠️ **`stop()` 的早退判据不是它**：看的是 `started_once_`（`src/webapp/uvcpp_web_app.cpp:2197`）—— 工作循环在 `init_process_once()` 里就被放行，会在 0 号还没建好自己的 `async` 时就把 `loop_started_` 变真，那时 `stop()` 就在一条没有 `async` 的槽位上干活了。 |
| `thread_` / `thread_started_` / `threading_` | `src/webapp/uvcpp_web_app.h:1803-1805` | 单数 ⇒ 向量。`join()`（`src/webapp/uvcpp_web_app.cpp:2235-2302`）、`start_background()`（`src/webapp/uvcpp_web_app.cpp:1610-1676`）、析构三处都按"一个线程"写。 |

**丙、每循环一份 —— 今天**全部**是单数成员**

前两条（身份与投递）与两个定时器那一族**已搬进 `loop_slot`**
（`src/webapp/uvcpp_web_app.h:1866-2091`，W4 / 2a + 2b 已落地）。注意成员名去掉了
尾下划线 —— 它们在结构体里，不再是类的直接成员：

| 成员 | 位置 |
|---|---|
| `loop_tid` / `tid_known` / `tid_mutex` | `src/webapp/uvcpp_web_app.h:1884` / `src/webapp/uvcpp_web_app.h:1885` / `src/webapp/uvcpp_web_app.h:1883` |
| `async` | `src/webapp/uvcpp_web_app.h:1888` —— **一个 `uv_async` 只能服务它注册的那条循环**，这条最硬 |
| `post_queue` / `post_mutex` | `src/webapp/uvcpp_web_app.h:1890` / `src/webapp/uvcpp_web_app.h:1889` |
| `loop` / `index` | `src/webapp/uvcpp_web_app.h:1880` / `src/webapp/uvcpp_web_app.h:1876` |
| `idle_timer` / `shutdown_timer` | `src/webapp/uvcpp_web_app.h:1899` / `src/webapp/uvcpp_web_app.h:1900` —— `uv_timer` 与 `async` 同一条理由：**循环亲和** |
| `shutdown_deadline_ms` / `shutdown_phase` | `src/webapp/uvcpp_web_app.h:1918` / `src/webapp/uvcpp_web_app.h:1919` |
...
| 成员 | 新位置 |
|---|---|
| `inflight` / `flushing` | `src/webapp/uvcpp_web_app.h:1961` / `src/webapp/uvcpp_web_app.h:2016` |
| `upgraded` | `src/webapp/uvcpp_web_app.h:2020` |
| `registry` | `src/webapp/uvcpp_web_app.h:1932` |
| `file_transfers` | `src/webapp/uvcpp_web_app.h:2023-2024` |
...
`src/webapp/uvcpp_web_app.cpp:2405-2425`）。理由是后者那些入口的调用者**不保证**在
...
> （`src/webapp/uvcpp_web_app.cpp:2470`）、`connection_count_at()`
> （`src/webapp/uvcpp_web_app.cpp:2476`）、`inflight_total()`
> （`src/webapp/uvcpp_web_app.cpp:822`）现在要**遍历每一格**的表 —— 即使在 0 号
...
> 是公开的（`src/webapp/uvcpp_web_app.h:1079`）⇒ `tcp_server()->set_loops(n)`
> 能绕过这一批进多循环，而上面这些边界在那条路上**全部成立**。在 step 3 正式接上
> 之前别那么用。

**丁、一份、但内部已经按循环切好了**：`http_`（`src/webapp/uvcpp_web_app.h:1391`）
—— `contexts_` 已切成 `vector<map>`（§4.1），net 层的 `clients_` 已按 worker 切。

**戊、只读常量，不分类**：`empty_chain_`（`src/webapp/uvcpp_web_app.h:1791`）。

**己、三个"每循环"的访问器**（不是成员，所以上面几张表都装不下，但同一类）：
`on_loop_thread()`（`src/webapp/uvcpp_web_app.cpp:2318`）、`post()`（`src/webapp/uvcpp_web_app.cpp:2324`）、
`loop()`（`src/webapp/uvcpp_web_app.cpp:2367`）—— 三个都在 `uvcpp_web_context_host` 接口上
（`src/webapp/uvcpp_web_context.h:101-142`），今天都用**唯一那条**循环作答。其中
`loop()` 有个不在显然处的调用点：`serve_static()` 的 handler 在**请求时**才要它
（`src/webapp/uvcpp_web_app.cpp:1359` 那行 `st->serve(req, resp, next, self->loop())`，
...
> `serve_static()`（`src/webapp/uvcpp_web_app.cpp:1323-1326`）造的是 `uvcpp_web_static`，
...
> —— `uvcpp_http_server::compress_variants_`（`src/web/uvcpp_http_server.h:1186-1186`）是
> **请求期惰性写**的缓存：命中时改 `last_used` / `compress_variant_clock_` 并计数
> （`src/web/uvcpp_http_server.cpp:1224-1224`），未命中时插入并可能触发 LRU 淘汰
> （`src/web/uvcpp_http_server.cpp:1277-1277`、`src/web/uvcpp_http_server.cpp:1079-1079`）。
> 多循环下这些写来自**多条循环线程** ⇒ 它和 `compress_variant_clock_` / `_hits_` /
> `_misses_` / `_stored_`（`src/web/uvcpp_http_server.h:1187-1190`）一起加锁；
> **按循环切不成立** —— 它本来就是跨循环共用的缓存，切了就退回每循环各自 deflate。
>
> **已落地**：`mutable std::mutex compress_mu_`（`src/web/uvcpp_http_server.h:1204-1204`）
> 一把**非递归**锁护住那张表与四个计数。加锁点只有三个**外层入口** —— 命中
> （`src/web/uvcpp_http_server.cpp:1224-1224`）、存入（含淘汰，**同一次临界区**：淘汰那句要读
> 整张表的字节总量，拆成两次加锁会让别的循环插在中间按一个已不成立的总量做决定）、
> `compress_variant_stats()`。两个帮手改名成 `..._locked()`
> （`src/web/uvcpp_http_server.h:1215-1215` / `src/web/uvcpp_http_server.h:1219-1219`），意思就是"调用方已持锁" ——
...
> （`src/webapp/uvcpp_web_app.h:1932`）今天正是默认构造的。n>1 那天，哪条循环忘了传
...
- **日志这条最重**：`src/webapp/uvcpp_log.cpp:416-436` 的 `write()` **已经不再把 sink
  调用关在锁里** —— 锁内只快照一次 `sink()`（`:342-351`），`should_log()` 与
  `target->write()` 都在锁外，所以**用户的 sink 不在全局锁里执行**（1.4.0 改的）；
  等级过滤也不再拿锁（relaxed 原子读，`:406-413`）。剩下的进程级共享是：`sink()` 那次
  快照要拿 `mutex_`（每记录一次，临界区只有一次指针读 + 可能的懒创建），以及默认 console
  sink **自己**那把锁 —— 锁内做 `std::string` 拼接与 `fwrite`。多循环下这是唯一一处
  **每条请求都可能**撞上的进程级锁。
...
`SSL_CTX` 是 per-app 的，`start()` 里交给每个 tcp_server（`src/webapp/uvcpp_web_app.cpp:1972`）。
...
⇒ 19 处全部改成"表取一次、拿它自己的 `end()` 比"；`src/web/uvcpp_http_server.cpp:1760-1760`
...
⇒ **整块循环内存泄漏**（正是 net 层 `uvcpp_loop_worker.cpp:163-164` 与
...
> **本循环**那张表（`src/web/uvcpp_http_server.cpp:1760-1760` 的 `ctxs_here()`），而
> `ctxs_at(-1)` 在 `contexts_.size() > 1` 且调用者不在任何循环线程上时直接
> `std::abort()`（`src/web/uvcpp_http_server.cpp:151-156`）⇒ **停机那一族必须"每条
> 循环各跑一次、且每次都在该循环自己的线程上"**。顺着这条往回看，今天的 `stop()`
> （`src/webapp/uvcpp_web_app.cpp:2190`）在 n>1 下是坏的：非循环线程进来时它走
...
> （`src/webapp/uvcpp_web_app.cpp:3448`）关的是**接受者的 listener 句柄**
...
- 新增 `post(fn, loop_index)`（`src/webapp/uvcpp_web_app.h:1187`）显式指定。
...
多循环只走 `start()` + `join()`。
`loop()`（`src/webapp/uvcpp_web_app.cpp:2367-2373`）**跟着改了**，而且是必须改的那一个：
它现在答**本线程那条循环**（`slot_here().loop`），不是"唯一那条"。§4.1.1 己 那条依赖
就是它 —— `serve_static()` 的 handler 在**请求时**才取 `loop()`（`src/webapp/uvcpp_web_app.cpp:1359`），
...
2. **聚合量仍然是聚合量**：`inflight_count()` 必须对**所有**循环的每个连接求和 —— 头文件里
   那条注释已经写明这一点（`src/webapp/uvcpp_web_app.h:1132`），切分时别改成读某一份。
   **[2c 已做]**：求和那一半在 `inflight_total()`（`src/webapp/uvcpp_web_app.cpp:822`）
   与 `connection_count()`（`src/webapp/uvcpp_web_app.cpp:2470`）里。
