...
（`src/net/uvcpp_tcp_server.h:309-310`）、`client_count_at(i)`
（`src/net/uvcpp_tcp_server.h:347`）。**不调用它或 `set_loops(1)` 与今天逐字节相同**
（`src/net/uvcpp_tcp_server.h:196` 起是完整的对外说明）。

```
acceptor 循环（0 号 = 今天的 loop_，调用者的线程跑它）
  └─ uv_connection_cb
       ├─ n == 1：今天那条路，逐字不动（src/net/uvcpp_tcp_server.cpp:267）
       └─ n > 1 ：accept 进一个**临时句柄**
                  （src/net/uvcpp_tcp_server.cpp:424-424 accept_and_handoff）
...
worker[k] 线程：loop[k].run(UV_RUN_DEFAULT)
  └─ 邮箱回调里排空（src/net/uvcpp_loop_worker.h:141-141 起）
       └─ 在**这条循环的线程**上：new uvcpp_tcp_client(worker_loop) 装转手来的 socket
          （src/net/uvcpp_tcp_server.cpp:472-472 on_handoff_task）→ mark_accepted() → 登记
          → setup_client_callbacks() → TLS 块 → deliver_connection()
          （用户的连接回调在这里被调用，src/net/uvcpp_tcp_server.cpp:499-499 finish_accept）
```
...
- **转手必须在 `mark_accepted()` 之前**，也就是整条尾巴（登记、`setup_client_callbacks`、
  ...
  循环上 arm 一次读（`src/net/uvcpp_tcp_server.cpp:532-532`）。
...
| `uvcpp_tcp_server::clients_` | `src/net/uvcpp_tcp_server.h:759` | 接受 / 关闭 / 计数 |
...
> （`src/webapp/uvcpp_web_app.h:1842-2010`）里，`loops_`（`src/webapp/uvcpp_web_app.h:2043-2043`）
> 是"每循环一格"的向量，`slot_here()`（`src/webapp/uvcpp_web_app.cpp:2334-2350`）
> 按**调用线程**取格 —— 认不出是哪条循环的线程时给 0 号（§5.2）。
> `n == 1` 走快路直接回 `loops_[0]`，**一次锁都不多加**，与今天逐字相同。
> `post()` 现在是 `src/webapp/uvcpp_web_app.cpp:2283-2295`，`loop()` 是
> `src/webapp/uvcpp_web_app.cpp:2326-2332`，`on_loop_thread()` 是
> `src/webapp/uvcpp_web_app.cpp:2277-2281`。
...
> 放行点是 `set_loops()` **自己的** `w->start()`（`src/net/uvcpp_tcp_server.cpp:357`），
...
> 返回 `UV_EBUSY`（`src/net/uvcpp_tcp_server.cpp:318-318`），而 accept 回调分叉看的
> 是 `!workers_.empty()`（`src/net/uvcpp_tcp_server.cpp:259-259`）—— 两句都预设
> "`set_loops()` 先跑完、线程已经在跑"。

**但"接受者就是这个循环"在那条路上不再成立 —— 这正是 net 层要转手的原因。**
`:266` 的 `new uvcpp_tcp_client(loop_)` 与 `:269` 的 `s->accept(...)` 今天是 **n==1 那条分支**；
n>1 时先把连接 accept 进一个**临时句柄**（`src/net/uvcpp_tcp_server.cpp:424-424` 的 `accept_and_handoff`），
客户端对象是在**目标工作循环的线程**上建的（`src/net/uvcpp_tcp_server.cpp:472-472` 的 `on_handoff_task`）。
...
> `chain_storage_.push_back`（`src/webapp/uvcpp_web_app.cpp:3192-3192`）与
> `chain_cache_[...]`（`src/webapp/uvcpp_web_app.cpp:3194-3194`）确实在 `start()` 里，但**那两个函数在请求路径上可达**：
> `sync_chains()`（`src/webapp/uvcpp_web_app.cpp:3144-3144`）被 `src/webapp/uvcpp_web_app.cpp:1236-1236`（流式那条路）与 `src/webapp/uvcpp_web_app.cpp:3077-3077`（派发前）调用，
> `build_chain()`（`src/webapp/uvcpp_web_app.cpp:3170-3170`）被 `src/webapp/uvcpp_web_app.cpp:1238-1238` 与 `src/webapp/uvcpp_web_app.cpp:3099-3099` 调用。让这些写**离开**请求路径的是
...
> `init_on_loop_thread()`（`src/webapp/uvcpp_web_app.cpp:2135-2135`）是**循环线程的入口**，
> 而访问日志那句 `middlewares_.insert(middlewares_.begin(), web_middleware_access_log())`
> （`src/webapp/uvcpp_web_app.cpp:1860-1860`）与 `++middleware_gen_`
> （`src/webapp/uvcpp_web_app.cpp:1861-1861`）**当天就在这个入口里**（今天已经搬走，见下面的已落地）。
> n>1 时这个函数**每条循环各跑一次** ⇒
> ① 插进 **n 份**访问日志中间件（每请求记 n 次。本库对这一档有读数：`off → info`
> 是 +17~19% rps、每请求 19 → 25 次分配）；
> ② 更要紧的是**竞态** —— `middlewares_` / `middleware_gen_` 在请求路径上被**读**
> （`src/webapp/uvcpp_web_app.cpp:3159-3159`、`src/webapp/uvcpp_web_app.cpp:3183-3185`、`src/webapp/uvcpp_web_app.cpp:3204-3204`/`src/webapp/uvcpp_web_app.cpp:3219-3219`/`src/webapp/uvcpp_web_app.cpp:3238-3238`），而 n 条循环**不同时起跑**：
...
`init_process_once()`（`src/webapp/uvcpp_web_app.cpp:1759-1759`），每循环那半仍是
`init_on_loop_thread()`（`src/webapp/uvcpp_web_app.cpp:2135-2135`）。两处调用点都按
...
（`src/webapp/uvcpp_web_app.cpp:1860-1860` 与 `src/webapp/uvcpp_web_app.cpp:1861-1861`）
当时落在循环线程的入口里，而所有者是**进程**（上面那条更正二）。
**2026-09-22 已落地**：这两句所属的函数现在是 `init_process_once()`
（`src/webapp/uvcpp_web_app.cpp:1759-1759`）—— **行号没动，换的是函数**。甲这一格其余各笔
...
| 成员 | 位置 | 为什么是进程级 |
|---|---|---|
| `bound_port_` | `src/webapp/uvcpp_web_app.h:2045-2045` | 一个 app 一个监听端口（`bind()` 只调一次）。 |
| `started_once_` | `src/webapp/uvcpp_web_app.h:1791-1791` | "这个 app 起过了"，与几条循环无关。 |
| `stopping_` | `src/webapp/uvcpp_web_app.h:1807-1807` | 它答的是"这次停机请求受理过了没有"（幂等用 `exchange`，`src/webapp/uvcpp_web_app.cpp:2160-2160`）。`shutdown_phase_` 才是"我这条循环走到第几步" —— **两者今天挨着写，很容易被一起搬**。 |
...
| `thread_` / `thread_started_` / `threading_` | `src/webapp/uvcpp_web_app.h:1779-1781` | 单数 ⇒ 向量。`join()`（`src/webapp/uvcpp_web_app.cpp:2194-2261`）、`start_background()`（`src/webapp/uvcpp_web_app.cpp:1577-1643`）、析构三处都按"一个线程"写。 |

**丙、每循环一份 —— 今天**全部**是单数成员**

前两条（身份与投递）与两个定时器那一族**已搬进 `loop_slot`**
（`src/webapp/uvcpp_web_app.h:1842-2010`，W4 / 2a + 2b 已落地）。注意成员名去掉了
...
| 成员 | 新位置 |
|---|---|
| `inflight` / `flushing` | `src/webapp/uvcpp_web_app.h:1925-1925` / `src/webapp/uvcpp_web_app.h:1935-1935` |
| `upgraded` | `src/webapp/uvcpp_web_app.h:1939-1939` |
| `registry` | `src/webapp/uvcpp_web_app.h:1908-1908` |
| `file_transfers` | `src/webapp/uvcpp_web_app.h:1942-1943` |
...
`src/webapp/uvcpp_web_app.cpp:2364-2384`）。理由是后者那些入口的调用者**不保证**在
...
> （`src/webapp/uvcpp_web_app.cpp:2429-2429`）、`connection_count_at()`
> （`src/webapp/uvcpp_web_app.cpp:2435-2435`）、`inflight_total()`
> （`src/webapp/uvcpp_web_app.cpp:807-807`）现在要**遍历每一格**的表 —— 即使在 0 号
...
`on_loop_thread()`（`src/webapp/uvcpp_web_app.cpp:2277-2277`）、`post()`（`src/webapp/uvcpp_web_app.cpp:2283-2283`）、
`loop()`（`src/webapp/uvcpp_web_app.cpp:2326-2326`）—— 三个都在 `uvcpp_web_context_host` 接口上
（`src/webapp/uvcpp_web_context.h:101-142`），今天都用**唯一那条**循环作答。其中
`loop()` 有个不在显然处的调用点：`serve_static()` 的 handler 在**请求时**才要它
（`src/webapp/uvcpp_web_app.cpp:1326-1326` 那行 `st->serve(req, resp, next, self->loop())`，
...
> `serve_static()`（`src/webapp/uvcpp_web_app.cpp:1290-1293`）造的是 `uvcpp_web_static`，
...
`SSL_CTX` 是 per-app 的，`start()` 里交给每个 tcp_server（`src/webapp/uvcpp_web_app.cpp:1931-1931`）。
...
`uvcpp_tcp_server.cpp:101-117` 记着的那一族）。
...
> （`src/webapp/uvcpp_web_app.cpp:2149-2149`）在 n>1 下是坏的：非循环线程进来时它走
...
> （`src/webapp/uvcpp_web_app.cpp:3332-3332`）关的是**接受者的 listener 句柄**
...
> 在 `src/net/uvcpp_tcp_server.cpp:655-655`，而第一次进来就在
> `src/net/uvcpp_tcp_server.cpp:661-661` `clear_status`），
...
（`src/net/uvcpp_tcp_server.cpp:267` 是 n==1、`src/net/uvcpp_tcp_server.cpp:260` 是 n>1），变异"绕开转手"
...
多循环只走 `start()` + `join()`。
`loop()`（`src/webapp/uvcpp_web_app.cpp:2326-2332`）**跟着改了**，而且是必须改的那一个：
它现在答**本线程那条循环**（`slot_here().loop`），不是"唯一那条"。§4.1.1 己 那条依赖
就是它 —— `serve_static()` 的 handler 在**请求时**才取 `loop()`（`src/webapp/uvcpp_web_app.cpp:1326-1326`），
...
2. **聚合量仍然是聚合量**：`inflight_count()` 必须对**所有**循环的每个连接求和 —— 头文件里
   ...
   **[2c 已做]**：求和那一半在 `inflight_total()`（`src/webapp/uvcpp_web_app.cpp:807-807`）
   与 `connection_count()`（`src/webapp/uvcpp_web_app.cpp:2429-2429`）里。
