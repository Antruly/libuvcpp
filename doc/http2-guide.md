...
2. **ALPN 必须已经协商成 `h2`。** connection 的构造函数**不校验**这一点
   ...
   （照 `src/web/uvcpp_http_server.cpp:256-256`）。
...
库内那份接好的实现（`src/web/uvcpp_http_server.cpp:1728-1728` 起）的调用序列：

1. 分流点判 ALPN，是 h2 就走另一条路（`src/web/uvcpp_http_server.cpp:256-256`）；
2. `new uvcpp_h2_connection(client, /*server_side=*/true)`（`src/web/uvcpp_http_server.cpp:1728-1728`）；
3. 装 `uvcpp_h2_session::callbacks`（`src/web/uvcpp_http_server.cpp:1733-1733`）与 `uvcpp_h2_connection::callbacks`（`src/web/uvcpp_http_server.cpp:1793-1793`）；
4. 挂框架自己的关闭回调 —— **最终 `delete` 在那里**（`src/web/uvcpp_http_server.cpp:1798-1798`）；
5. `h2->start(h2c, cc)`（`src/web/uvcpp_http_server.cpp:1800-1800`）；失败就 `h2->close_now()`（`src/web/uvcpp_http_server.cpp:1801-1801`）；
6. 业务处理完之后 `h2->send_response(stream_id, resp, omit_body)`（`src/web/uvcpp_http_server.cpp:1838-1838` 起）。
...
这一点：`src/web/uvcpp_http_server.cpp:1755-1756` 明说那里**不能** `erase` 流状态，
...
`src/web/uvcpp_http_server.h:960-963` 提到的那个 idle sweep 属于 **webapp 层**
