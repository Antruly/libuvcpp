...
2. **ALPN 必须已经协商成 `h2`。** connection 的构造函数**不校验**这一点
   ...
   （照 `src/web/uvcpp_http_server.cpp:273-273`）。
...
（`src/net/uvcpp_tcp_client.h:221-221` / `:224` / `:236` / `:258`）。
...
库内那份接好的实现（`src/web/uvcpp_http_server.cpp:1799-1799` 起）的调用序列：

1. 分流点判 ALPN，是 h2 就走另一条路（`src/web/uvcpp_http_server.cpp:273-273`）；
2. `new uvcpp_h2_connection(client, /*server_side=*/true)`（`src/web/uvcpp_http_server.cpp:1799-1799`）；
3. 装 `uvcpp_h2_session::callbacks`（`src/web/uvcpp_http_server.cpp:1804-1804`）与 `uvcpp_h2_connection::callbacks`（`src/web/uvcpp_http_server.cpp:1864-1864`）；
4. 挂框架自己的关闭回调 —— **最终 `delete` 在那里**（`src/web/uvcpp_http_server.cpp:1869-1869`）；
5. `h2->start(h2c, cc)`（`src/web/uvcpp_http_server.cpp:1871-1871`）；失败就 `h2->close_now()`（`src/web/uvcpp_http_server.cpp:1872-1872`）；
6. 业务处理完之后 `h2->send_response(stream_id, resp, omit_body)`（`src/web/uvcpp_http_server.cpp:1909-1909` 起）。
...
这一点：`src/web/uvcpp_http_server.cpp:1826-1827` 明说那里**不能** `erase` 流状态，
...
`src/web/uvcpp_http_server.h:1009-1012` 提到的那个 idle sweep 属于 **webapp 层**
