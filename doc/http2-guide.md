...
库内那份接好的实现（`src/web/uvcpp_http_server.cpp:1775-1775` 起）的调用序列：

1. 分流点判 ALPN，是 h2 就走另一条路（`src/web/uvcpp_http_server.cpp:273-273`）；
2. `new uvcpp_h2_connection(client, /*server_side=*/true)`（`src/web/uvcpp_http_server.cpp:1775-1775`）；
3. 装 `uvcpp_h2_session::callbacks`（`src/web/uvcpp_http_server.cpp:1780-1780`）与 `uvcpp_h2_connection::callbacks`（`src/web/uvcpp_http_server.cpp:1840-1840`）；
4. 挂框架自己的关闭回调 —— **最终 `delete` 在那里**（`src/web/uvcpp_http_server.cpp:1845-1845`）；
5. `h2->start(h2c, cc)`（`src/web/uvcpp_http_server.cpp:1847-1847`）；失败就 `h2->close_now()`（`src/web/uvcpp_http_server.cpp:1848-1848`）；
6. 业务处理完之后 `h2->send_response(stream_id, resp, omit_body)`（`src/web/uvcpp_http_server.cpp:1885-1885` 起）。
...
这一点：`src/web/uvcpp_http_server.cpp:1802-1803` 明说那里**不能** `erase` 流状态，
...
`src/web/uvcpp_http_server.h:988-991` 提到的那个 idle sweep 属于 **webapp 层**
