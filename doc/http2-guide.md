...
库内那份接好的实现（`src/web/uvcpp_http_server.cpp:1378` 起）的调用序列：

1. 分流点判 ALPN，是 h2 就走另一条路（`src/web/uvcpp_http_server.cpp:165`）；
2. `new uvcpp_h2_connection(client, /*server_side=*/true)`（`:1378`）；
3. 装 `uvcpp_h2_session::callbacks`（`:1383`）与 `uvcpp_h2_connection::callbacks`（`:1439`）；
4. 挂框架自己的关闭回调 —— **最终 `delete` 在那里**（`:1444`）；
5. `h2->start(h2c, cc)`（`:1446`）；失败就 `h2->close_now()`（`:1447`）；
6. 业务处理完之后 `h2->send_response(stream_id, resp, omit_body)`（`:1483` 起）。
...
这一点：`src/web/uvcpp_http_server.cpp:1403-1404` 明说那里**不能** `erase` 流状态，
