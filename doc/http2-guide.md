...
库内那份接好的实现（`src/web/uvcpp_http_server.cpp:1639-1639` 起）的调用序列：

1. 分流点判 ALPN，是 h2 就走另一条路（`src/web/uvcpp_http_server.cpp:247-247`）；
2. `new uvcpp_h2_connection(client, /*server_side=*/true)`（`src/web/uvcpp_http_server.cpp:1639-1639`）；
3. 装 `uvcpp_h2_session::callbacks`（`src/web/uvcpp_http_server.cpp:1644-1644`）与 `uvcpp_h2_connection::callbacks`（`src/web/uvcpp_http_server.cpp:1704-1704`）；
4. 挂框架自己的关闭回调 —— **最终 `delete` 在那里**（`src/web/uvcpp_http_server.cpp:1709-1709`）；
5. `h2->start(h2c, cc)`（`src/web/uvcpp_http_server.cpp:1711-1711`）；失败就 `h2->close_now()`（`src/web/uvcpp_http_server.cpp:1712-1712`）；
6. 业务处理完之后 `h2->send_response(stream_id, resp, omit_body)`（`src/web/uvcpp_http_server.cpp:1749-1749` 起）。
...
这一点：`src/web/uvcpp_http_server.cpp:1666-1667` 明说那里**不能** `erase` 流状态，
