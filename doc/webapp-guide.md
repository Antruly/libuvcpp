...
**它的形状是"一条接受者 + n−1 条工作循环"，不是"n 条抢同一个端口"。**
只有 0 号那条循环 `bind()` + `listen()`；连进来的连接由接受者**显式轮转**着转手给
工作循环 —— 第 i 条连接落 `1 + (i % (n-1))`。所以 `n = 2` 时**全部**连接都落在 1 号
循环上，0 号只收不发。（Windows 上没有 `SO_REUSEPORT`，抢端口那条路本来也走不通。）
