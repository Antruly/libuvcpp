...
17. [安全](#17-安全)
18. [已知限制](#18-已知限制)
19. [横向扩展：多循环（`set_loops`）](#19-横向扩展多循环set_loops)
...
- **超限时的行为：既不排队也不拒绝** —— 它只回答"有没有名额"（`acquire()` 返回 bool），
  怎么处理由调用方按自己的退路决定：
  - **静态文件·整读**（文件 ≤ `max_cached_file_size`）：`acquire()` 拿不到就回
    **503 + `Retry-After`**。取名额发生在 worker 里**真正要读盘的那一步之前**，所以不读盘
    的分支（LRU 命中 / 404 / 403）**不占名额**、也不会被回绝；反过来说超限的请求**已经
    进了线程池**了，这道闸门不是投递前的准入（被回绝的那些只做了一次 `stat`，代价与卡住
    线程池队列的那点排队是两回事）。
  - **静态文件·分片**（文件 > 阈值，`1.3.6-dev` 起）：**不拒**，改由传输自己**按块借还**
    （`set_chunk_gate()`）—— 读一块之前拿、读完立刻还，拿不到就停在块边界上等
    `resume()`。这条路上**没有 503**：名额封的是"真正并行的读盘数"，并发再高也只是慢。
    改的理由：这条支原先在 worker 里取名额、持到 `after_work`，而 `after_work` 跑在
    **循环线程**上 ⇒ 名额被"一个字节都还没读"的请求攥着。现场是服务端自己那行
    `工作池已满（在途 0 / 16）`（**在途 0 却拒绝**）；`ab -c 1000` 打 8 MiB 文件、4000
    个请求，改前 503 有 **267** 个，改后 **0** 个。
...
## 19. 横向扩展：多循环（`set_loops`）

一条循环吃满一个核之后，加核的办法是开多条循环。`uvcpp_web_app::set_loops(n)` 就是它。

```cpp
uvcpp_web_app app;
app.set_host("0.0.0.0").set_port(8080);
int rc = app.set_loops(4);   // 不调用 = 1 = 今天的行为
if (rc != 0) return rc;      // UV_EINVAL 参数越界 / UV_EBUSY 装晚了
app.get("/hello", handler);
app.start();                 // 起 4 条循环线程
app.join();                  // 等 4 条都退出（有界，见下）
```

**"新连接怎么落到某一条循环上"分两种形状**（运行时探测；`tcp_server()->is_fanout()`
告诉你这次是哪一种）：

- **内核分流**（平台接受 `UV_TCP_REUSEPORT`，Linux 是）：n 条循环**各自 `bind()` 同一个
  端口**，内核按四元组分给其中任意一条 —— **0 号自己也收属于它的那一份**。`n = 2` 时
  两条循环都在收，分布由内核哈希定、**不保证均匀**，只保证逐格之和 ==
  `connection_count()`。
- **接受者 + 转手**（Windows，以及那个标志被拒时的回落）：只有 0 号那条循环 `bind()` +
  `listen()`，连进来的连接由它**显式轮转**着转手给工作循环 —— 第 i 条连接落
  `1 + (i % (n-1))`。所以 `n = 2` 时**全部**连接都落在 1 号循环上，0 号只收不发。

**判据别共用**：转手路上"逐循环条数相等"是确定的（显式轮转），分流路上只能断言守恒。
见[多循环横向扩展设计](./multiloop-design.md) §6 第 4 条。

### 返回错误码，所以不能链式

`set_loops()` 返 `int`，与上面那一串链式 setter 不同 —— 它有**两个真实的失败**要说：

| 返回 | 什么时候 |
|---|---|
| `UV_EINVAL` | `n` 不在 `1..64` |
| `UV_EBUSY` | 已经 `start()` 过；或底层 `uvcpp_tcp_server` 已经先 `set_loops(n>1)` 了 |

为什么"装晚了"要报错而不是静默退化：多循环是**结构性**的开关（要开 n−1 条线程、
每格建自己的句柄）。退化成一个不像样的配置，比启动失败坏得多 —— 用户会以为
"我开了 4 条循环"，实际只有一条在干活，而且这一点从外部**看不出来**（能跑、能响应、
非 2xx 请求数 0，与成功逐字相同）。

### 启动失败（`n > 1`）

`set_loops(n>1)` **一返回，那 n−1 条工作循环就已经在跑了**，而 `start()` 里还有会
失败的步骤（最常见的是 `bind()`：端口被占）。这一段窗口由框架自己收尾：

- **失败 ⇒ 没有线程残留**：`start()` 返回非 0 时，已经放行的工作循环被停掉并释放，
  `loop_started()` / `running()` 都是假，析构照常。
- **但同一个实例不能再 `start()`**：重试等于把整条启动路径重走一遍，而那条路径上
  "每进程那半"（`bind()`/`listen()` 的状态、一次性的启动标记）不是为复用设计的 ⇒
  第二次 `start()` 返 `UV_EBUSY` + 一条说明原因的 ERROR，**要重试就析构重建**（`n == 1` 无此限制）。

为什么"起不来"要收得这么死：多循环下它本来有两条路，一条是**当场报错**（端口被占，
用户看得见），另一条是**照常返回 0、却有一格永远进不了停机状态机**（那格的
`uv_async` 建失败 ⇒ 它的投递全被丢弃 ⇒ `stop()` 停不掉它，只有 `join()` 的有界等待
超时之后报一句"仍有 N 条循环没退出"）。后者从外部与成功长得一模一样，所以这一批
把它归到前者：**`start()` 返回 0 ⇒ 那 n−1 格的句柄都建成了 ⇒ 停机不会卡在"某格不
响应"上**。

### 停机

`stop()` 是**线程安全**的，逐条循环扇出；`join()` 等**全部**循环退出，但等待是
**有界的**：预算是 `shutdown_grace_ms`（默认 3000 ms，`set_shutdown_grace_ms()` 改）
再加 5 s 的余量。超时会打一条 ERROR

```
join() 已等 N ms，仍有 M 条循环没退出（停机扇出或某条循环的收尾没走到）。……
```

然后返回 —— 那几格的句柄**不再回收**（宁可泄漏，也好过挂住）。所以**"`join()` 返回了"
不等于"循环都停干净了"**，要看有没有那条 ERROR。

### 看得见分布

多循环最怕的失败是"只有一个循环真干活"，而它与成功长得一模一样。所以框架把**逐循环**
的读数公开出来：

| 读 | 含义 |
|---|---|
| `app.loop_count()` | 当前设定的循环数（没调 `set_loops()` = 1） |
| `app.connection_count_at(i)` | **第 i 条循环**上的连接数（i 越界答 0） |
| `app.connection_count()` | 全部循环之和 |
| `app.inflight_count()` | 全部循环的在途请求数之和 |

后两个是**聚合量**：它们读的是每格的原子计数，所以**可以在循环跑着的时候**从任何
线程读。要判断"平不平"用 `connection_count_at(i)`，别拿聚合量去推。

### 例外与代价（用之前该知道的）

- **DEBUG 档停机日志多一行**：`finish_shutdown()` 会打一条带循环号的
  `停机完成：循环 N 即将退出`。这是"逐循环停自己那条循环"唯一的每循环可观测面，
  所以它是**明知而留下**的例外 —— `n = 1` 时默认档（INFO 及以上）**一个字节都不变**。
- **`run()` 不接受 `n > 1`**（返 `UV_EINVAL`）：它是"在调用者线程上就地跑 0 号循环"，
  那 n−1 条工作循环的归属就不成立了。多循环走 `start()` + `join()`。
- **跨线程读登记表仍未做**：`app.connection(id)` / `app.find(id)` 会定位到 id 所属的
  那一格，但那一格里的 `std::map` 不是线程安全的。聚合量不受影响（见上表）。
- **运行中注册路由在 `n > 1` 时是 UB**：链缓存全进程共享，n 条循环同时读写它。
  单循环时最坏是"命中一条过期链"，多循环时是数据竞争。**注册路由要在 `start()` 之前**，
  这条契约在 `n > 1` 之后从"建议"变成"必须"。
- **静态缓存在多循环下也照加锁**（`n = 1` 时同样加）：那份缓存是跨循环**共用**的，
  按循环切会成倍吃内存还掉命中率。代价是一次无竞争的 lock/unlock。

设计稿（含两种形状的取舍、"一条接受者 + 转手"的由来、以及那些没做的）见
[多循环横向扩展设计](./multiloop-design.md)；net 层那一半的用法见
[net 网络层指南](./net-guide.md) 的多循环一节。
...
[多进程横向扩展设计](./worker-process-design.md)（仍是设计稿，未实现）、
[多循环横向扩展设计](./multiloop-design.md)（`net` 层与 `webapp` 层都已落地）。
