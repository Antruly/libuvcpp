...
（`uvcpp_tcp_client.cpp:594-598`）——**同一个协议层客户端对象根本连不了第二次**。
...
- **超限时的行为：既不排队也不拒绝** —— 它只回答"有没有名额"（`acquire()` 返回 bool），
  怎么处理由调用方按自己的退路决定：
  - **静态文件**：`acquire()` 拿不到就当场回 **503 + `Retry-After`**，一个线程池任务都不投
    （请求已整包读完，没有退路）。
...
| **`run()` 在"没有活句柄"时才自然返回** | 客户端连着、或对端已走但循环里还有句柄（例如关掉重连之后）时，`run()` **不会自己返回**，得由使用者 `stop()`。所以用 `run()` 的形态请自己安排收场（`on_close` 里决定 `stop()`）。`uvcpp_web_ws_client::run()` 的另一条出口是 `stopping_`。 |
