...
| 落点 | 声明 | 所有权说明 |
|---|---|---|
| `uvcpp_tcp_server::set_ssl_context` | `src/net/uvcpp_tcp_server.h:481` | "生命周期必须覆盖**整个服务端**，本服务端不持有它的所有权，也不负责释放"（`:474-476`） |
| `uvcpp_tcp_client::enable_tls` | `src/net/uvcpp_tcp_client.h:221-221` | "生命周期必须覆盖**整条连接**"（`:217`） |
| `uvcpp_web_app` | `src/webapp/uvcpp_web_app.cpp:1737-1737` | 全库唯一"有人拥有"的一处：`std::shared_ptr<uvcpp_ssl_context>` |
...
- 服务端：握手成功才 `deliver_connection()`；失败**只记账**，`on_connection`
  **一次都不被调用**（`src/net/uvcpp_tcp_server.h:465-473`）。
