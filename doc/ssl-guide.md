...
| `uvcpp_web_app` | `src/webapp/uvcpp_web_app.cpp:1962-1962` | 全库唯一"有人拥有"的一处：`std::shared_ptr<uvcpp_ssl_context>` |
...
（`src/net/uvcpp_tcp_client.cpp:2392-2392`）——所以**必须看各 `load_*` 的返回值**，
不能只看 `is_ready()`。
