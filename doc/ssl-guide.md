...
`generate_self_signed` **只用于测试和受控内网**（`src/webapp/uvcpp_web_app.h:451-454`）。
...
| `uvcpp_web_app` | `src/webapp/uvcpp_web_app.cpp:1947-1947` | 全库唯一"有人拥有"的一处：`std::shared_ptr<uvcpp_ssl_context>` |
...
（`src/net/uvcpp_tcp_client.cpp:2176-2176`）——所以**必须看各 `load_*` 的返回值**，
不能只看 `is_ready()`。
