...
外部贡献者的台架上实测：**87 次死亡 / 2 170 885 条连接**；打上预期修复之后 **55 → 0**。
...
> 数并列。扩展性读数（外部贡献者本机量到 **3.29× / 3.45×**）只在同一台机器上可比。
> 上面那个 25% 的折扣**不要乘进 3.29×** —— 把它乘上去会得到 2.4× 量级，但那一步只有在
> **本库这套形状上跑 `set_loops(N)`** 才算数，不是这么推的。
