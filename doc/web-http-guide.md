...
- `set_stream_claim(hook)`（`:263`）：`post_stream` 没中时问它。它返回一个
  ...
  接进来的（`src/webapp/uvcpp_web_app.cpp:1856-1856`）。
...
（`src/webapp/uvcpp_web_app.cpp:2786-2792`）。
...
（`src/webapp/uvcpp_web_app.cpp:205`），而且为 0 时连扫描句柄都不建
（`src/webapp/uvcpp_web_app.cpp:2088-2093`）。升级成 WebSocket 的连接被排除在闲置超时之外
