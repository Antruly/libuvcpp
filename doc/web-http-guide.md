...
（`src/web/uvcpp_http_server.h:1075` 的 `contexts_` —— 装的是**每连接状态**，
...
（`src/web/uvcpp_http_server.h:966-966`）—— 单个连接只能从 `uvcpp_tcp_client` 那侧关。
...
- 都没命中 → `default_handler_`；`default_handler_` 为空 → 直接 **404**
  （`src/web/uvcpp_http_server.cpp:650-655`）。
...
框架见到它就直接 return（`src/web/uvcpp_http_server.cpp:661-662`），
...
（`:1047-1053`）。`is_connected()` 就是 `generation != 0`（`:575`）。
...
| 在 h2 连接上调 h1 那两个重载 | `write_stream` 返回 `UV_EINVAL`、`end_stream` **静默返回**（`src/web/uvcpp_http_server.cpp:967-967`、`src/web/uvcpp_http_server.cpp:996-996`）/ `begin_stream(client, resp)` 打一条 stderr 后丢弃（`src/web/uvcpp_http_server.cpp:868-873`） | — |
...
（`src/web/uvcpp_http_server.cpp:1887-1895`）。
...
**三个上限的默认值全是 0，也就是全都不限**（`src/web/uvcpp_http_server.h:1049-1051`）：
...
zlib 编进来时**压缩默认开**（`src/web/uvcpp_http_server.h:1127-1127`），最小 body 1024
（`src/web/uvcpp_http_server.h:1128-1128`），排除的 MIME 走 `default_excluded_mime_types()`
（`src/web/uvcpp_http_compress.cpp:146-165`）。压缩变体缓存三道限：
单条 ≤ 4 MiB、总量 ≤ 32 MiB、条数 ≤ 1024（`src/web/uvcpp_http_server.cpp:1016-1026`）。

### 协议行为

- **keep-alive**：HTTP/1.1 默认开、1.0 默认关；handler 显式设了 `connection`
  头就听 handler 的（`src/web/uvcpp_http_server.cpp:771-785`）。
- **`Date` 每个响应都发，且按秒缓存**：四个出报文的口子各补一次 ——
  `send_response`（`src/web/uvcpp_http_server.cpp:756-756`）、h1 流式
  `begin_stream`（`src/web/uvcpp_http_server.cpp:899-899`）、h2 流式
  `begin_stream`（`src/web/uvcpp_http_server.cpp:932-932`）、h2 整包
  `send_h2_response`（`src/web/uvcpp_http_server.cpp:1879-1879`）。值由
...
- **`Expect: 100-continue` 支持**，且在 headers 回调里就补
  ...
  被忽略（`src/web/uvcpp_http_server.cpp:670-675`）；**未知的 `Expect` 回 417**（`src/web/uvcpp_http_server.cpp:685-690`）。
- **声明的 `Content-Length` 超限 → body 还没到就回 413**（`src/web/uvcpp_http_server.cpp:693-705`）。
...
3. **返回值加 stderr**：`send_response` 在连接已不在表里时**打一条 stderr 警告
   并返回 0**（`src/web/uvcpp_http_server.cpp:740-750`）；`begin_stream` 同理
   （`src/web/uvcpp_http_server.cpp:851-858`）—— 但它是 `void`，你**无从判断**。

**异常只在一处被接住**：`on_connection` / `stream_claim` / h2 连接钩子三个钩子
里抛出的异常会被吞掉并打 stderr（`src/web/uvcpp_http_server.cpp:295-306`、`src/web/uvcpp_http_server.cpp:382-393`、`src/web/uvcpp_http_server.cpp:1743-1752`）。
**`handler(req, resp, client)` 本身没有 try/catch**（`src/web/uvcpp_http_server.cpp:650-655`），
...
`src/web/uvcpp_http_server.cpp:511-512`、`src/web/uvcpp_http_server.cpp:1419-1421` 与头文件
`src/web/uvcpp_http_server.h:981-984` 现在都明写这一点。
