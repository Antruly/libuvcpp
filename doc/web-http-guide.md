...
之上（`src/web/uvcpp_http_server.cpp:48-50`：构造函数第一件事就是
`new uvcpp_tcp_server()`），每条连接配一个独立的 `uvcpp_http_parser` 上下文
（`src/web/uvcpp_http_server.h:1009` 的 `contexts_` —— 装的是**每连接状态**，
...
`uvcpp_tcp_server`（默认 10000 ms，`src/net/uvcpp_tcp_server.h:692`）。
...
返回 libuv 错误码（`src/web/uvcpp_http_server.cpp:141-170`）。`listen` 的默认 backlog 是
...
（`src/net/uvcpp_tcp_server.h:390-403`）。已经在处理的请求照跑。
...
（`src/web/uvcpp_http_server.h:900-900`）—— 单个连接只能从 `uvcpp_tcp_client` 那侧关。
...
**匹配规则（`src/web/uvcpp_http_server.cpp:215-228`）：剥掉 query 再逐字节比路径，
线性扫描，首次命中即返回。** 于是：
...
- 都没命中 → `default_handler_`；`default_handler_` 为空 → 直接 **404**
  （`src/web/uvcpp_http_server.cpp:586-591`）。
- **分不出 404 和 405**：`get("/x")` 已注册时来一个 `POST /x`，走的还是兜底
  或 404，不会回 405。

路由表就是个 `std::vector`（`src/web/uvcpp_http_server.cpp:190-213`），
`listen()` 之后仍可以改；但它没有锁。

### 请求目标里的 query 要自己解

**web 层不提供任何 query / cookie / form / URL 解码工具，`req.url` 就是原始请求目标
（含 `?query`）。** 路由匹配时会把 query 剥掉再去比（`src/web/uvcpp_http_server.cpp:217-220`），
...
- `set_stream_claim(hook)`（`:263`）：`post_stream` 没中时问它。它返回一个
  ...
  接进来的（`src/webapp/uvcpp_web_app.cpp:1889-1889`）。
...
框架见到它就直接 return（`src/web/uvcpp_http_server.cpp:597-598`），
...
（`:981-987`）。`is_connected()` 就是 `generation != 0`（`:575`）。
...
（`src/web/uvcpp_http_server.cpp:66-68` 明说）。
...
| 在 h2 连接上调 h1 那两个重载 | `write_stream` 返回 `UV_EINVAL`、`end_stream` **静默返回**（`src/web/uvcpp_http_server.cpp:903-903`、`src/web/uvcpp_http_server.cpp:932-932`）/ `begin_stream(client, resp)` 打一条 stderr 后丢弃（`src/web/uvcpp_http_server.cpp:804-809`） | — |
...
（`src/web/uvcpp_http_server.cpp:1757-1765`）。

### 升级到别的协议

`on_upgrade(handler)`（`src/web/uvcpp_http_server.h:235`）是单槽（`src/web/uvcpp_http_server.cpp:176-178`）。
它被调时这条连接已经从 http 解析里摘出来了，之后的字节归你；升级时**已经读进来但
还没解析的字节**用 `take_upgrade_leftover(client)`（`src/web/uvcpp_http_server.h:250-250` / `src/web/uvcpp_http_server.cpp:180-180`）取走 ——
...
**三个上限的默认值全是 0，也就是全都不限**（`src/web/uvcpp_http_server.h:983-985`）：
...
（`src/web/uvcpp_http_server.cpp:257-258`）⇒ **listen 之后改它只对新连接生效**；
`max_body_size_` 是每块 body 现读成员（`src/web/uvcpp_http_server.cpp:318-318`）⇒ **立刻生效**。
三处头文件注释一个字都没提这个区别。
...
是在 `headers_complete` 才被问的（`src/web/uvcpp_http_server.cpp:325-379`）。
所以被认领的请求**照样会被 414/431 拒掉并直接关连接**，认领者连 `HEADERS` 事件
都收不到。**只有 `max_body_size` 真正停在认领边界**（`src/web/uvcpp_http_server.cpp:308-324`）——
...
单条 ≤ 4 MiB、总量 ≤ 32 MiB、条数 ≤ 1024（`src/web/uvcpp_http_server.cpp:952-962`）。

### 协议行为

- **keep-alive**：HTTP/1.1 默认开、1.0 默认关；handler 显式设了 `connection`
  头就听 handler 的（`src/web/uvcpp_http_server.cpp:707-721`）。
- **`Date` 每个响应都发，且按秒缓存**：四个出报文的口子各补一次 ——
  `send_response`（`src/web/uvcpp_http_server.cpp:692-692`）、h1 流式
  `begin_stream`（`src/web/uvcpp_http_server.cpp:835-835`）、h2 流式
  `begin_stream`（`src/web/uvcpp_http_server.cpp:868-868`）、h2 整包
  `send_h2_response`（`src/web/uvcpp_http_server.cpp:1749-1749`）。值由
  ...
  **恒 GMT**，星期名与月份名是写死的 ASCII 表 —— 它**不**用 `strftime` 的
  ...
  （`src/web/uvcpp_http_server.cpp:398-398`）与 WebSocket 握手
...
- **`Expect: 100-continue` 支持**，且在 headers 回调里就补
  ...
  被忽略（`src/web/uvcpp_http_server.cpp:606-611`）；**未知的 `Expect` 回 417**（`src/web/uvcpp_http_server.cpp:621-626`）。
- **声明的 `Content-Length` 超限 → body 还没到就回 413**（`src/web/uvcpp_http_server.cpp:629-641`）。
- **畸形报文走正规响应路径回 400 + `Connection: close`**（`src/web/uvcpp_http_server.cpp:479-488`），
  不是手搓字节。
- **流水线**：一次读里可以有多条报文，解析器按消息边界 reset
  （`src/web/uvcpp_http_server.cpp:283-305`、`src/web/uvcpp_http_server.cpp:427-443`）；**web 层不限条数**，webapp 才限。
...
3. **返回值加 stderr**：`send_response` 在连接已不在表里时**打一条 stderr 警告
   并返回 0**（`src/web/uvcpp_http_server.cpp:676-686`）；`begin_stream` 同理
   （`src/web/uvcpp_http_server.cpp:787-794`）—— 但它是 `void`，你**无从判断**。

**异常只在一处被接住**：`on_connection` / `stream_claim` / h2 连接钩子三个钩子
里抛出的异常会被吞掉并打 stderr（`src/web/uvcpp_http_server.cpp:269-280`、`src/web/uvcpp_http_server.cpp:354-365`、`src/web/uvcpp_http_server.cpp:1613-1622`）。
**`handler(req, resp, client)` 本身没有 try/catch**（`src/web/uvcpp_http_server.cpp:586-591`），
...
`src/web/uvcpp_http_server.cpp:469-470`、`src/web/uvcpp_http_server.cpp:1276-1278` 与头文件
...
（`src/webapp/uvcpp_web_app.cpp:2121-2126`）。升级成 WebSocket 的连接被排除在闲置超时之外
