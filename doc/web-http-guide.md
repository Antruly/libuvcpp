...
- `set_stream_claim(hook)`（`:263`）：`post_stream` 没中时问它。它返回一个
  ...
  接进来的（`src/webapp/uvcpp_web_app.cpp:1905-1905`）。
...
（`src/webapp/uvcpp_web_app.cpp:2846-2852`）。
...
（`src/webapp/uvcpp_web_app.cpp:213-213`），而且为 0 时连扫描句柄都不建
（`src/webapp/uvcpp_web_app.cpp:2137-2142`）。升级成 WebSocket 的连接被排除在闲置超时之外
