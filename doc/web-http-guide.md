...
- 都没命中 → `default_handler_`；`default_handler_` 为空 → 直接 **404**
  （`src/web/uvcpp_http_server.cpp:501-506`）。
...
框架见到它就直接 return（`src/web/uvcpp_http_server.cpp:512-513`），
...
| 在 h2 连接上调 h1 那两个重载 | `write_stream` 返回 `UV_EINVAL`、`end_stream` **静默返回**（`src/web/uvcpp_http_server.cpp:801`、`:829`）/ `begin_stream(client, resp)` 打一条 stderr 后丢弃（`:712-717`） | — |
...
（`src/web/uvcpp_http_server.cpp:1514-1522`）。
...
单条 ≤ 4 MiB、总量 ≤ 32 MiB、条数 ≤ 1024（`src/web/uvcpp_http_server.cpp:849-859`）。

### 协议行为

- **keep-alive**：HTTP/1.1 默认开、1.0 默认关；handler 显式设了 `connection`
  头就听 handler 的（`src/web/uvcpp_http_server.cpp:616-630`）。
- **`Expect: 100-continue` 支持**，且在 headers 回调里就补
  ...
  被忽略（`src/web/uvcpp_http_server.cpp:521-526`）；**未知的 `Expect` 回 417**（`src/web/uvcpp_http_server.cpp:536-541`）。
- **声明的 `Content-Length` 超限 → body 还没到就回 413**（`src/web/uvcpp_http_server.cpp:544-556`）。
...
3. **返回值加 stderr**：`send_response` 在连接已不在表里时**打一条 stderr 警告
   并返回 0**（`src/web/uvcpp_http_server.cpp:592-600`）；`begin_stream` 同理
   （`src/web/uvcpp_http_server.cpp:696-702`）—— 但它是 `void`，你**无从判断**。

**异常只在一处被接住**：`on_connection` / `stream_claim` / h2 连接钩子三个钩子
里抛出的异常会被吞掉并打 stderr（`src/web/uvcpp_http_server.cpp:187-198`、`:272-283`、`:1382-1391`）。
**`handler(req, resp, client)` 本身没有 try/catch**（`src/web/uvcpp_http_server.cpp:501-506`），
...
`src/web/uvcpp_http_server.cpp:386-387`、`:1118-1120` 与头文件
...
它们是 `on_request_complete` 的局部对象（`src/web/uvcpp_http_server.cpp:462`、`:481`），
