...
`#include <webapp/...>`。反过来 `webapp/` 大量包含 `web/`（`src/webapp/uvcpp_web_app.h:128-129`
...
之上（`src/web/uvcpp_http_server.cpp:51-53`：构造函数第一件事就是
`new uvcpp_tcp_server()`），每条连接配一个独立的 `uvcpp_http_parser` 上下文
（`src/web/uvcpp_http_server.h:1054` 的 `contexts_` —— 装的是**每连接状态**，
...
就被登记并装好收尾（`src/net/uvcpp_tcp_server.h:53-58`），关闭时框架摘除并 `delete`
（`src/net/uvcpp_tcp_server.cpp:129-131`）。http 层那张 `contexts_` 表**不拥有**
...
`uvcpp_tcp_server`（默认 10000 ms，`src/net/uvcpp_tcp_server.h:717`）。
...
返回 libuv 错误码（`src/web/uvcpp_http_server.cpp:150-179`）。`listen` 的默认 backlog 是
...
（`src/net/uvcpp_tcp_server.h:415-428`）。已经在处理的请求照跑。
...
（`src/web/uvcpp_http_server.h:945-945`）—— 单个连接只能从 `uvcpp_tcp_client` 那侧关。
...
**匹配规则（`src/web/uvcpp_http_server.cpp:224-237`）：剥掉 query 再逐字节比路径，
线性扫描，首次命中即返回。** 于是：
...
- 都没命中 → `default_handler_`；`default_handler_` 为空 → 直接 **404**
  （`src/web/uvcpp_http_server.cpp:620-625`）。
- **分不出 404 和 405**：`get("/x")` 已注册时来一个 `POST /x`，走的还是兜底
  或 404，不会回 405。

路由表就是个 `std::vector`（`src/web/uvcpp_http_server.cpp:199-222`），
`listen()` 之后仍可以改；但它没有锁。

### 请求目标里的 query 要自己解

**web 层不提供任何 query / cookie / form / URL 解码工具，`req.url` 就是原始请求目标
（含 `?query`）。** 路由匹配时会把 query 剥掉再去比（`src/web/uvcpp_http_server.cpp:226-229`），
...
- `set_stream_claim(hook)`（`:263`）：`post_stream` 没中时问它。它返回一个
  ...
  接进来的（`src/webapp/uvcpp_web_app.cpp:1926-1926`）。
...
框架见到它就直接 return（`src/web/uvcpp_http_server.cpp:631-632`），
...
（`:1026-1032`）。`is_connected()` 就是 `generation != 0`（`:575`）。
...
（`src/web/uvcpp_http_server.cpp:69-71` 明说）。
...
| 在 h2 连接上调 h1 那两个重载 | `write_stream` 返回 `UV_EINVAL`、`end_stream` **静默返回**（`src/web/uvcpp_http_server.cpp:937-937`、`src/web/uvcpp_http_server.cpp:966-966`）/ `begin_stream(client, resp)` 打一条 stderr 后丢弃（`src/web/uvcpp_http_server.cpp:838-843`） | — |
...
（`src/web/uvcpp_http_server.cpp:1857-1865`）。

### 升级到别的协议

`on_upgrade(handler)`（`src/web/uvcpp_http_server.h:235`）是单槽（`src/web/uvcpp_http_server.cpp:185-187`）。
它被调时这条连接已经从 http 解析里摘出来了，之后的字节归你；升级时**已经读进来但
还没解析的字节**用 `take_upgrade_leftover(client)`（`src/web/uvcpp_http_server.h:250-250` / `src/web/uvcpp_http_server.cpp:189-189`）取走 ——
...
（`src/web/uvcpp_static_server.h:100-101`）—— 上面那个写法（`files` 先于 `server` 声明、
后于它析构）是对的。

默认值（构造参数 `src/web/uvcpp_static_server.h:93-95`、成员初值 `:190-191`）：
`url_root = "/"`、`index_file = "index.html"`、`spa_fallback = true`、`cache_enabled = true`。
后两个**有 setter**（`set_spa_fallback()` `:108`、`set_cache_enabled()` `:111`），
前两个只能走构造函数。
...
**三个上限的默认值全是 0，也就是全都不限**（`src/web/uvcpp_http_server.h:1028-1030`）：
...
（`src/web/uvcpp_http_server.cpp:266-267`）⇒ **listen 之后改它只对新连接生效**；
`max_body_size_` 是每块 body 现读成员（`src/web/uvcpp_http_server.cpp:327-327`）⇒ **立刻生效**。
三处头文件注释一个字都没提这个区别。
...
是在 `headers_complete` 才被问的（`src/web/uvcpp_http_server.cpp:334-388`）。
所以被认领的请求**照样会被 414/431 拒掉并直接关连接**，认领者连 `HEADERS` 事件
都收不到。**只有 `max_body_size` 真正停在认领边界**（`src/web/uvcpp_http_server.cpp:317-333`）——
...
zlib 编进来时**压缩默认开**（`src/web/uvcpp_http_server.h:1106-1106`），最小 body 1024
（`src/web/uvcpp_http_server.h:1107-1107`），排除的 MIME 走 `default_excluded_mime_types()`
（`src/web/uvcpp_http_compress.cpp:146-165`）。压缩变体缓存三道限：
单条 ≤ 4 MiB、总量 ≤ 32 MiB、条数 ≤ 1024（`src/web/uvcpp_http_server.cpp:986-996`）。

### 协议行为

- **keep-alive**：HTTP/1.1 默认开、1.0 默认关；handler 显式设了 `connection`
  头就听 handler 的（`src/web/uvcpp_http_server.cpp:741-755`）。
- **`Date` 每个响应都发，且按秒缓存**：四个出报文的口子各补一次 ——
  `send_response`（`src/web/uvcpp_http_server.cpp:726-726`）、h1 流式
  `begin_stream`（`src/web/uvcpp_http_server.cpp:869-869`）、h2 流式
  `begin_stream`（`src/web/uvcpp_http_server.cpp:902-902`）、h2 整包
  `send_h2_response`（`src/web/uvcpp_http_server.cpp:1849-1849`）。值由
  ...
  **恒 GMT**，星期名与月份名是写死的 ASCII 表 —— 它**不**用 `strftime` 的
  ...
  （`src/web/uvcpp_http_server.cpp:407-407`）与 WebSocket 握手
...
- **`Expect: 100-continue` 支持**，且在 headers 回调里就补
  `HTTP/1.1 100 Continue\r\n\r\n` 裸字节（`src/web/uvcpp_http_server.cpp:393-408`）—— 注释解释了为什么
  不能用 `send_response()`（1xx 不该带 `Content-Length`）。HTTP/1.0 上 `Expect`
  被忽略（`src/web/uvcpp_http_server.cpp:640-645`）；**未知的 `Expect` 回 417**（`src/web/uvcpp_http_server.cpp:655-660`）。
- **声明的 `Content-Length` 超限 → body 还没到就回 413**（`src/web/uvcpp_http_server.cpp:663-675`）。
- **畸形报文走正规响应路径回 400 + `Connection: close`**（`src/web/uvcpp_http_server.cpp:502-511`），
  不是手搓字节。
- **流水线**：一次读里可以有多条报文，解析器按消息边界 reset
  （`src/web/uvcpp_http_server.cpp:292-314`、`src/web/uvcpp_http_server.cpp:450-466`）；**web 层不限条数**，webapp 才限。
...
3. **返回值加 stderr**：`send_response` 在连接已不在表里时**打一条 stderr 警告
   并返回 0**（`src/web/uvcpp_http_server.cpp:710-720`）；`begin_stream` 同理
   （`src/web/uvcpp_http_server.cpp:821-828`）—— 但它是 `void`，你**无从判断**。

**异常只在一处被接住**：`on_connection` / `stream_claim` / h2 连接钩子三个钩子
里抛出的异常会被吞掉并打 stderr（`src/web/uvcpp_http_server.cpp:278-289`、`src/web/uvcpp_http_server.cpp:363-374`、`src/web/uvcpp_http_server.cpp:1713-1722`）。
**`handler(req, resp, client)` 本身没有 try/catch**（`src/web/uvcpp_http_server.cpp:620-625`），
抛出去会穿过 llhttp 的 C 回调栈。webapp 层自己包了 try/catch 并扇出到错误中间件
（`src/webapp/uvcpp_web_app.cpp:2877-2883`）。
...
`src/web/uvcpp_http_server.cpp:492-493`、`src/web/uvcpp_http_server.cpp:1389-1391` 与头文件
`src/web/uvcpp_http_server.h:960-963` 现在都明写这一点。
**真实的闲置清扫在 webapp 层**：`idle_timeout_ms` 默认 **60000**
（`src/webapp/uvcpp_web_app.cpp:221-221`），而且为 0 时连扫描句柄都不建
（`src/webapp/uvcpp_web_app.cpp:2158-2163`）。升级成 WebSocket 的连接被排除在闲置超时之外
（`src/webapp/uvcpp_web_app.h:817`）。

**二、`req` 与 `resp` 只在本次调用期间有效。**
它们是 `on_request_complete` 的局部对象（`src/web/uvcpp_http_server.cpp:570-570`、`src/web/uvcpp_http_server.cpp:598-598`），
