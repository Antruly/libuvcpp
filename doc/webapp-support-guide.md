...
`uvcpp_web_conn_id` 是一个 `uint64_t`（`webapp/uvcpp_web_connection.h:57`），
从 1 开始，**单调递增、永不复用**；0 是无效值
（`UVCPP_WEB_INVALID_CONN_ID`，`:60`）。存在它的唯一理由是：
...
形态只有第一个实参。登记表全部方法在 `webapp/uvcpp_web_connection.h:139-263`。
...
`web_sanitize_path()` 是**纯文本、不碰文件系统**（`uvcpp_web_util.h:299-302`），
...
（`uvcpp_web_util.h:276-277` 带默认实参的 explicit 构造函数），所以
...
`uvcpp_web_mime_map` 存在的两个理由（`webapp/uvcpp_web_mime.h:10-19`）：
...
内置表**只有一份**：`web_mime_builtin_table()`（`uvcpp_web_util.h:438`，实现
`uvcpp_web_util.cpp:979-982`），`builtin_size()` 也转调它
（`uvcpp_web_mime.cpp:134-138`）。所以"同一个文件在 `web_mime_type()` 和静态服务里
类型不一样"这类漂移在结构上被排除了 —— 值得知道，因为这类漂移很难查。

`default_map()`（`uvcpp_web_mime.h:126`）是**进程级共享**的静态表，静态服务没显式
...
**没有 IO、不碰 libuv、不碰请求对象**（`webapp/uvcpp_web_multipart.h:13-16`）。
...
**默认上限只写在 `.cpp` 里，头文件一个都没写**（`uvcpp_web_multipart.cpp:288-293`）：
...
`webapp/uvcpp_web_file.h:26-38`）。它刻意不拉 libuv（fd 用 `int` 存，`:323`）。
...
（`webapp/uvcpp_web_context.h:58-63`），要原始连接得显式走 `raw_client()`。
...
`post()` 在 loop 线程上**就地同步执行**（`uvcpp_web_context.cpp:60-70`），不能假设
fn 在"下一个循环迭代"跑。
...
（`webapp/uvcpp_log_console.h:6-9`）。`webapp-guide.md` §16 已经讲了
...
- **`set_sink()` 不接管所有权**（`src/webapp/uvcpp_log.h:224-229`）：`&sink` 必须比
  用法活得久。内置 sink 由 logger 自己 `new`/`delete`（`uvcpp_log.cpp:256-258`、
  `:273`、`:342`），**用户传进去的 sink 不会被 delete**。
- **`split_streams` 默认 true** ⇒ WARN 及以上走 `stderr` 并且会 `fflush`
  （`uvcpp_log_console.cpp:228-235`）。这是"WARN 日志不会因为进程被杀而丢"的实现点。
...
```cpp
// doc-snippet: fragment — 反面例子，故意编不过；照抄 uvcpp_log_console.h:25-26 的注释写出来的
uvcpp_console_log_options opt{false, true};
```

`webapp/uvcpp_log_console.h:25-26` 的注释说"用显式构造函数而不是成员初始化器 …… 
...
**对照组**：`web_path_options`（`uvcpp_web_util.h:276-277`）是这一族里唯一写对的
—— 它的构造函数带默认实参，所以花括号写法可用。

### `color` 的默认值不是"自动判断"

`uvcpp_log_console.h:29` 写 `color` 的默认是"按终端能力自动判断"，
而结构体的实际默认值是 **`true`**（`uvcpp_log_console.cpp:124`）。能力探测在 sink 的
...
`uvcpp_log_console.h:43-45` 的样例是

```
2026-09-13 12:34:56.789 [INFO ] [REQUEST] GET /index.html  (server.cpp:88)
```

而 `show_thread` 默认 **true**（`uvcpp_log_console.cpp:126`）⇒ 真实输出在 message
之前还有一个 ` (tid:N)`（`:209-214`）。要复现样例得显式 `opt.show_thread = false`。

### `on_done` 不一定来自 `uv_fs_close`

`uvcpp_web_file.h:140-141` 说 on_done"由 `uv_fs_close` 的完成回调调用"。
实际有三条**没有 close 回调**的路径直接调 `finish()`：open 失败
（`uvcpp_web_file.cpp:210-216` → `finish()`；`fd_ < 0` 时 `:333-337` 直接收尾）、
...
`uvcpp_web_util.h:128-138` 只说"拆 path/query + 剥 `#fragment`"，实现还会**先剥掉
绝对形式（代理风格）的 `http://host`**（`uvcpp_web_util.cpp:204-208`）——
`GET http://x/../y HTTP/1.1` 这种请求目标。头文件没写这一条。

### `.cpp` 里那条注释是错的

`uvcpp_web_mime.cpp:113` 的注释声称 `lookup("a.b/c")` 看的是 `c`，实测得到 `b`：
实现先取**最后一个点**再截到第一个分隔符（`:110-124`），而 `a.b/c` 里最后一个点在
目录名中。头 `uvcpp_web_mime.h:103-104` 举的 `archive.tar.gz` 与 `lib..min.js`
两个例子是**对的**，错的是 `.cpp` 那句。
...
| 类型 | 约束 | 出处 |
|---|---|---|
| `uvcpp_web_connection_registry` | **只能 loop 线程**（内部 `std::map` 无锁） | `uvcpp_web_connection.h:30-31`、`:120` |
| `uvcpp_web_context` | 除 `post()` 外都在 loop 线程 | `uvcpp_web_context.h:98-99` |
| `uvcpp_web_file_transfer` | loop 线程驱动，回调都在 loop 线程 | 头里**没写**这条 |
| `uvcpp_web_util` | 纯函数，无状态、线程安全 | — |
| `uvcpp_web_mime_map` | 注册期配置、运行期只读；`default_map()` 是全局的 | `uvcpp_web_mime.h:31-34` |
| `uvcpp_web_multipart` | **无线程设施**，由调用方决定在哪条线程跑 | `uvcpp_web_multipart.h:13-16` |
| `uvcpp_console_log_sink` | `write()` 可从工作线程调用；`options()` 的读取**无锁** | `uvcpp_log.h:135-140`；`uvcpp_log_console.cpp:163-169` |

在别的线程上要查连接，先 `context::post()` 回 loop 线程
（`uvcpp_web_connection.h:31`）。改 `options()` 而 `write()` 正在别的线程上跑是
**数据竞争** —— 选项只在构造时定，或者只在 loop 线程改。

### 所有权

- `uvcpp_web_file_transfer` **必须 `shared_ptr`**（`uvcpp_web_file.h:164`）。
...
- `uvcpp_web_connection_registry::find()` 返回内部 `std::map` 里的指针
  （`uvcpp_web_connection.cpp:195-201`）。
- `uvcpp_web_mime_map::lookup()` 返回表内 `std::string` 的指针
  （`uvcpp_web_mime.cpp:107-108`）。
- `uvcpp_logger::set_sink()` **不接管所有权**（`uvcpp_log.h:224-229`）。
...
（`uvcpp_web_util.cpp:125-130`），`web_parse_http_date()` 失败返回 `(time_t)-1`
...
它调 `fail()`（`uvcpp_web_multipart.cpp:316-322`）→ 置 `ERROR_STATE` 并清空保留缓冲
...
（`uvcpp_web_multipart.cpp:363`、`:368`），文件/字段**数量**也是 `> 0` 才生效
...
`uvcpp_web_multipart.cpp:648` 那句 `received_ += len` 在终态判断**之前**。
...
一个字节都不读（`uvcpp_web_file.h:209-211`）。

**所有 setter 都在 `start()` 之前调。** `set_stop_at_eof` 头里写了"必须在 `start()`
之前设"（`uvcpp_web_file.h:213-214`）但 setter 无守卫；`set_slice_bytes()` 更危险
—— 它内部 `slice_buf_.resize()`（`uvcpp_web_file.cpp:101`），在途读期间调用会让
`uv_fs_read` 写进已释放的缓冲。

**`cancel()` 在"因背压停读"这一支上不能省。** 那时**没有任何 fs 操作在途**，
`cancel()` 必须自己推进状态机，否则 `on_done` 永远不来、fd 一直开着
（`uvcpp_web_file.cpp:197-203`）。`start()` 之前 `cancel()` 是空操作且**不触发
`on_done`**（`uvcpp_web_file.h:250-253`）。

**`backlog()` 是唯一的背压信号。** 恒返回 0 的 sink 只是让背压失效，
窗口退化成"切片缓冲那一份"（`uvcpp_web_file.h:126-131`）—— 允许，但不是有界的了。

**`web_url_encode()` 的 `keep` 优先于 `plus_for_space`。**
`uvcpp_web_util.cpp:177` 先判 `unreserved || keep.find(c) != npos`，**再**判
`c == ' ' && plus_for_space`。所以 `keep` 里放空格或 `+` 会让它们**原样输出** ——
查询串里一个原样输出的 `+` 会被对端解成空格。头 `:107-119` 没提这个优先级。
只放行 `/` 是安全的。

**`web_build_cookie()` 只校验 name 与 value。**
`path`（`uvcpp_web_util.cpp:469-472`）与 `same_site`（`:481-486`）是**原样拼接**的
...
照过），也**永远不能拿去拼路径**（`uvcpp_web_util.h:397-401`）；`max_len` 是**软**
上限（设备名保护会让结果多一字节，`:393-395`）。

**`web_parse_query()` 只按 `&` 切**（`uvcpp_web_util.cpp:252-282`）—— `;` 会被当成
...
- **`web_collapse_slashes()` 刻意不做**：不解码、不处理 `.` / `..`（与路由切段保持
  一致，挡目录穿越要用 `web_sanitize_path()`，`uvcpp_web_util.h:149-152`）。
- **`web_join_root()` 不做安全判断**（`uvcpp_web_util.h:324`），判断在
  `web_is_within_root()`（`:329-339`，实现用"前缀 + 分隔符"而不是裸前缀比较）。
- **其他 `multipart/*` 子类型不支持**（`mixed` 等）：`web_multipart_boundary()` 返回
  空串让调用方拒绝（`uvcpp_web_util.h:196-199`）。
- **multipart 只认 CRLF**（`uvcpp_web_multipart.h:69-73`）：裸 LF 在边界处不接受、
  在部件头里直接 400 —— 理由是请求走私。`[LWSP]` 上限 128 字节
  （`uvcpp_web_multipart.h:60-62`），超了明确失败而不是当 body。
- **multipart 不做文件名清洗**（`uvcpp_web_multipart.h:76-83`），`filename` 永远不能
  拼路径。
- **`uvcpp_web_file` 不知道 HTTP 的存在**：断连即释放**不在这里**，接线层调
  `cancel()`（`uvcpp_web_file.h:70-71`）。
- **卡住的出站流没有超时保护**（`uvcpp_web_context.h:356-358`，代价照实记在
  `uvcpp_web_context.cpp:353-358`）。
- **`attach_stream()` 的"必须在链跑起来之前调"没有运行时守卫**
  （`uvcpp_web_context.h:281-293`）：中途挂只会得到一个**永远收不到数据**的 stream，
  静默。
- **如实记一处代价**：`uvcpp_web_file.cpp:173-179` 承认同步失败路径会漏一份
