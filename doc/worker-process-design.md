...
（`src/webapp/uvcpp_web_app.cpp:1770-1770` → `src/web/uvcpp_http_server.cpp:140-141` →
...
- **不解决 `contexts_` 那一族。** 多进程形态下它们天然正确：每个 worker 一份，就是今天的
  ...
  `uvcpp_web_app` 的 `upgraded` / `inflight`（`src/webapp/uvcpp_web_app.h:1738-1738` /
  `src/webapp/uvcpp_web_app.h:1724-1724`；这两个容器后来在多循环那条路上被搬进了
