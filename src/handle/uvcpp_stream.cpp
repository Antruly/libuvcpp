...
uvcpp_stream::uvcpp_stream() {
  uv_stream_t* stream = uvcpp::uv_alloc<uv_stream_t>();
...
int uvcpp_stream::read_start(
    ...
    std::function<void(uvcpp_stream *, ssize_t, const uv_buf *)> read_cb) {
  handle_alloc_cb = alloc_cb;
  stream_read_cb = read_cb;
  return uv_read_start(UVCPP_STREAM_HANDLE, uvcpp_handle::callback_alloc, callback_read);
}

int uvcpp_stream::write(uvcpp_write *req, const uv_buf bufs[], unsigned int nbufs,
...
#if UV_VERSION_MAJOR >= 1
int uvcpp_stream::try_write(const uv_buf bufs[], unsigned int nbufs) {
  return uv_try_write(UVCPP_STREAM_HANDLE, reinterpret_cast<const ::uv_buf_t *>(bufs), nbufs);
}
#else
int uvcpp_stream::try_write(const uv_buf bufs[], unsigned int nbufs) {
...
int uvcpp_stream::try_write(const uv_buf bufs[], unsigned int nbufs,
                      uvcpp_stream *send_handle) {
  return uv_try_write2(UVCPP_STREAM_HANDLE, reinterpret_cast<const ::uv_buf_t *>(bufs), nbufs,
...
void uvcpp_stream::callback_read(uv_stream_t *handle, ssize_t nread,
  ...
  if (!wrapper || !wrapper->stream_read_cb)
    return;
  // Pass the libuv buffer directly as uv_buf (no copy).
  const uv_buf *view = reinterpret_cast<const uv_buf *>(buf);
  wrapper->stream_read_cb(wrapper, nread, view);
}
