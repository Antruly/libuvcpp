...
class UVCPP_API uvcpp_tcp_server {
 public:
   ...
   * **n 条循环各跑各的，这一点两端一致；两端不一致的是"新连接怎么落到某一条
   * 循环上"**，而且那件事是**运行时探测**出来的（同一个二进制在不同平台上不
   * 一样），不是编译期分叉。两种形状：
   *
   * - **内核分流**（平台支持 `UV_TCP_REUSEPORT` 时，Linux 是）：n 条循环各自绑
   *   一个监听句柄、抢同一个端口，内核按四元组把新连接分给其中任意一个 ——
   *   **0 号自己也收属于它的那一份**，就地收下，不转手。哪条连接落哪条循环由
   *   内核哈希决定：**不保证均匀、也不保证每条循环都能分到**，保证的只有"总数
   *   守恒、每条连接只落一处"（`sum(client_count_at(i)) == client_count()`）。
   * - **接受者 + 转手**（Windows；以及平台不支持那个标志时的回落）：0 号只接受
   *   和分发，每条连接按**显式轮转**交给 `1..n-1` 号，**一条不留在接受者上**；
   *   第 i 条连接必然落到 `1 + (i % (n-1))`，分布是确定的。
   *
   * 想知道这次是哪种：**`is_fanout()`**。这与可观测行为直接相关
   * （分流下 `client_count_at(0)` 非 0，`on_connection` 也会跑在 0 号线程上），
   * 所以别把两端写进同一套判据。回落发生时只在 stderr 打一行提示，不改变
   * 返回值 —— `set_loops()` 的成与不成不看这个。
   *
  ...
  /**
   * @brief 这次 `n > 1` 走的是**哪条形状**：`true` = 内核分流（n 条循环各自
   *        绑一个监听句柄抢同一个端口，0 号也承载连接），`false` = 接受者 +
   *        转手（0 号一条都不留）。
   *
   * **运行时探测的结果，不是编译期常量**：同一个二进制在不同平台上可能不同
   * （`UV_TCP_REUSEPORT` 在 Linux 上给得出来，在 Windows 上 libuv 无条件拒绝，
   * 在 macOS 上 `uv__sock_reuseport()` 没有实现）。判据要看这个值决定断言哪
   * 一套，别拿 `#ifdef` 猜平台。
   *
   * `n == 1`（含从没 `set_loops()` 过）时恒为 `false` —— 那条路上没有分流可
   * 言，0 号就地接受并处理，与不调 `set_loops()` 逐字节相同。
   *
   * 绑定成功之后才有意义（`bind()` 之前恒为 `false`）。
   */
   ...
   *
   * **0 号不是恒 0 的**：`is_fanout()` 为真时 0 号（就是 `get_loop()` 那条）
   * 自己也收属于它的那一份（见 `set_loops()` 那段）。越界返回 0。
   *
   * 它就是"分布看得见"的那个读数，但**分布的形状按平台分叉**：转手那条路上
   * 去向是显式轮转（连接数固定时这个数是确定的、逐格均分）；分流那条路上由
   * 内核哈希定（**只保证逐格之和 == `client_count()`**）。别把两端写进同一套
   * 判据 —— 想知道这次是哪一种就问 `is_fanout()`。
   */
