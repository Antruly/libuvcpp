...
class UVCPP_API uvcpp_web_static {
 public:
   ...
   * @brief 装上工作池在途上限。
   *
   * 装上之后，worker 在**真正要读盘的那一步之前**先要一个名额。拿不到名额时
   * **按走哪一支分两种**（`1.3.6-dev` 起）：
   *
   * - **整读进内存那一支**（文件 ≤ `max_cached_file_size`）：回
   *   **503 + `Retry-After`**。名额的取/还仍在 `run_job()` / `after_work()` 里。
   * - **超过阈值走分片那一支**：**不拒**，改由传输自己**按块借还**
   *   （`uvcpp_web_file_transfer::set_chunk_gate()`）—— 读一块之前拿、读完
   *   立刻还，拿不到就停在块边界上等唤醒。于是名额封的是"真正并行的读盘数"，
   *   并发再高也只是**慢**，不是失败。
   *
   * **为什么分片那一支必须单独处理**：它原先在 worker 里取名额、持到
   * `after_work`（跑在**循环线程**上），于是名额会被"一个字节都还没读"的请求
   * 攥着 —— 服务端自己那行日志就是现场：`工作池已满（在途 0 / 16），拒绝静态
   * 请求 /big.bin`（**在途 0 却拒绝**）。实测 `ab -c 1000` 打 8 MiB 文件，
   * 4000 个请求里 503 有 **267** 个；改成按块借还之后同一装置是 **0** 个，
   * rps **73 → 161**（同一批、同一台机）。
   *
   * 取名额的位置是 `run_job()` 里那条读盘语句之前，**不是**投递任务之前 ——
   * "要不要读盘"只有 stat 完才知道（命中缓存、404 都不用读），在投递前判会
   * 把那些请求一起 503 掉。整读那一支 503 的产出位置是 `finish_job()` 里的
   * 一个 `REJECTED` 分支（细节见 `uvcpp_web_static.cpp`）。
   *
   * 为什么**整读**那一支只能拒绝、不能背压：走到那里时请求**已经整包读完**了
   * （静态路由是普通路由，body 早收完），字节不在网上，没有"让对端慢点发"
   * 这种退路 —— 它没有"停在块边界"这种形状，因为它是**一次**读完的。分片支
   * 相反，它本来就分块，而那条续做通道是为背压建的，顺带就能承载名额等待。
   * 上传路径同理 —— 那里字节还在读，所以它的分工是 `stream->pause()`，见
   * `uvcpp_web_work_limit` 的说明。
   *
