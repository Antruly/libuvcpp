 ...
 *
 * 拿不到名额时怎么办，**按路径分工**（这不是一个可以统一的选择，而且同一类
 * 请求里"要不要真读盘"这一步也会把答案分成两种）
 * --------------------------------------------------------------------------
 * | 路径 | 处理 | 理由 |
 * |---|---|---|
 * | 静态文件·整读 | **503 + `Retry-After`** | 请求已经整包读完了，没有退路：不投递就无事可做，而"堆着不投"只会把内存堆爆 |
 * | 静态文件·分片（`1.3.6-dev` 起） | **停在块边界，等 `resume()`** | 分片路在块与块之间**本来就有一条为背压建的"续做"通道**，所以它有退路：停下什么都不占，名额还回去就行 |
 * | 上传（3b） | **`stream->pause()`** | 字节还在网上没读完，**能把压力退回去**：暂停读就是让对端等，这比拒绝好得多 |
 *
 * 所以本类不做"排队"也不做"拒绝"，它只回答"有没有名额"，怎么处理由调用方
 * 按自己的退路决定。
 *
 * **名额的粒度是调用方的事，而它不是无所谓的一件小事。** `1.3.6-dev` 之前，
 * 静态分片路是"一条传输占一个名额、持到 `after_work`"——而 `after_work` 跑在
 * **循环线程**上，于是 1000 条并发大文件时循环线程搬运不过来、名额就被"一个
 * 字节都还没读"的请求攥着。判据是服务端自己那行日志：
 *
 *     工作池已满（在途 0 / 16），拒绝静态请求 /big.bin
 *
 * **在途 0 却拒绝** —— 日志是事后在循环线程上读的，那时名额早还光了，差的就是
 * "持有者其实没在读盘"这一点。现在改成按块借还（读一块之前拿、读完立刻还），
 * 名额封的才是"真正并行的读盘数"。同一装置（`ab -c 1000` 打 8 MiB 文件，4000
 * 个请求）配同一台机的一批读数：503 **267 → 0**，rps **73 → 161**。
 *
