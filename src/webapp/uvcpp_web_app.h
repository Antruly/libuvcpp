...
class UVCPP_API uvcpp_web_app : public uvcpp_web_context_host {
 ...
 private:
  ...
  struct loop_slot {
    ...
    /**
     * @brief 一条请求用完的**空**头表，给这条连接的下一条请求复用。
     *
     * 与上面那两张表同一族（"内容不要、缓冲要"），但形状不同：这里只有**一块**
     * 表，不排队、也不设上限 —— 一条连接上同一时刻只有一个"当前请求"在搬头
     * （流水线里在途的那些各自已经把元素搬进自己的上下文了，见
     * `uvcpp_web_request::take_from()`），所以一块就够。
     *
     * 为什么要它：`uvcpp_web_request::src_.headers` 是**逐元素**接住头表的
     * （不是移动赋值 —— 那样会把 HTTP 层那块连接级缓冲换手带走），接住的地方
     * 必须自己有容量，否则这次接住本身就是一次分配。
     *
     * 槽里放的是 `yield_headers()` **清空过**的表，搬过来的永远是"空的、有容量的"。
     * 与那一族同一条理由：只在**这条循环的线程**上动，不需要锁。
     */
    http_headers req_hdr_recycle;
